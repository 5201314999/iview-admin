	...
	<div class="wrapper">
		<div class="ivu-transfer">
          ...
          <span class="ivu-transfer-list-header-title">
            <Checkbox v-show="!searchLeftFlag" v-model="checkAll_left"
                      @on-change="toggleAll($event,'dataLeft','checkedLeft',$refs.leftList,'checkAll_left')"
                      :disabled="dataLeft.length ==0">未选</Checkbox>
            <Checkbox v-show="searchLeftFlag" v-model="checkAll_left_match"
                      @on-change="toggleAll($event,'dataLeft_match','checkedLeft_match',$refs.leftList_match,'checkAll_left_match')"
                      :disabled="dataLeft_match.length==0">未选</Checkbox>
          </span>
				</div>
				...
				<div class="filter" v-if="dataOptions && dataOptions.length">
					<Select class="sourceSelector" v-model="data_type" size="small" style="width:100px"
					        @on-change="dataTypeChange">
						<Option v-for="(item,index) in dataOptions" :value="item.value" :key="index">{{ item.name }}
						</Option>
					</Select>
				</div>
				<div class="search-wrapper">
					<Input ref="searchLeft" v-model="keyword_left" icon="ios-search" size="small"
					       @on-change="searchChange($event,dataLeft,'dataLeft_match')" clearable></Input>
				</div>
				<div class="ivu-transfer-list-body">
					<Tree ref="leftList" v-show="!searchLeftFlag" :data="dataLeft" show-checkbox
					      class="ivu-transfer-list-content treeNode"
					      @on-check-change="checkChange('checkedLeft',$refs.leftList,'checkAll_left')"></Tree>
					<Tree ref="leftList_match" v-show="searchLeftFlag" :data="dataLeft_match" show-checkbox
					      class="ivu-transfer-list-content treeNode"
					      @on-check-change="checkChange('checkedLeft_match',$refs.leftList_match,'checkAll_left_match')"></Tree>
				</div>
			</div>
			<!-- 中间 -->
          ...
          <span class="ivu-transfer-list-header-title" v-show="!readonly">
            <Checkbox v-show="!searchRightFlag" v-model="checkAll_right"
                      @on-change="toggleAll($event,'dataRight','checkedRight',$refs.rightList,'checkAll_right')"
                      :disabled="dataRight.length ==0">{{config && config.secondSelect ? "未选" :"已选"}}</Checkbox>
            <Checkbox v-show="searchRightFlag" v-model="checkAll_right_match"
                      @on-change="toggleAll($event,'dataRight_match','checkedRight_match',$refs.rightList_match,'checkAll_left_match')"
                      :disabled="dataRight_match.length ==0">{{config && config.secondSelect ? "未选" :"已选"}}</Checkbox>
          </span>
            ...
            <span class="selected_count">已选</span>{{selected.length}}
					...
					<div class="ivu-transfer-list-header-title" v-show="readonly"
					     style="min-height:18px;text-align:center;">{{title}}
					</div>
				...
				<div class="search-wrapper">
					<Input ref="searchRight" v-model="keyword_right" icon="ios-search" size="small"
					       @on-change="searchChange($event,dataRight,'dataRight_match')" clearable></Input>
				</div>
				<div class="ivu-transfer-list-body">
					<Tree ref="rightList" v-show="!searchRightFlag" :data="dataRight" :show-checkbox="!readonly"
					      class="ivu-transfer-list-content treeNode"
					      @on-check-change="checkChange('checkedRight',$refs.rightList,'checkAll_right')"></Tree>
					<Tree ref="rightList_match" v-show="searchRightFlag" :data="dataRight_match"
					      :show-checkbox="!readonly" class="ivu-transfer-list-content treeNode"
					      @on-check-change="checkChange('checkedRight_match',$refs.rightList_match,'checkAll_right_match')"></Tree>
				</div>
			</div>
			<!-- 二次选择 -->
			<div ref="second-select-wrapper" class="second-select-wrapper" v-if="config && config.secondSelect">
				...
				<div class="ivu-transfer-operation">
					...
					<Button size="small" :disabled="disableRemove_2th()" @click="remove_2th">删除</Button>
				</div>
				<!-- 二次已选 -->
				<div class="ivu-transfer-list">
					<div class="ivu-transfer-list-header">
            <span class="ivu-transfer-list-header-title">
              <Checkbox v-show="!search_2th" v-model="checkAll_2th"
                        @on-change="toggleAll($event,'data_2th','checked_2th',$refs.list_2th,'checkAll_2th')"
                        :disabled="data_2th.length ==0">已选</Checkbox>
              <Checkbox v-show="search_2th" v-model="checkAll_2th_match"
              ...
              <span class="selected_count">已选</span>{{selected_2th.length}}
					...
					<div class="search-wrapper">
						<Input ref="searchRight" v-model="keyword_2th" icon="ios-search" size="small"
						       @on-change="searchChange($event,data_2th,'data_2th_match')" clearable></Input>
					</div>
					<div class="ivu-transfer-list-body">
						<Tree ref="list_2th" v-show="!search_2th" :data="data_2th"
						      class="ivu-transfer-list-content treeNode" show-checkbox
						      @on-check-change="checkChange('checked_2th',$refs.list_2th,'checkAll_2th')"></Tree>
						<Tree ref="list_2th_match" v-show="search_2th" :data="data_2th_match"
						      class="ivu-transfer-list-content treeNode" show-checkbox
						      @on-check-change="checkChange('checked_2th_match',$refs.list_2th_match,'checkAll_2th_match')"></Tree>
					</div>
				</div>
			</div>
		</div>
        ...
        methods: {
            ...
            _setSelected(ids) {
                ...
                if (ids && ids.length) {
                    ...
                    for (let i = 0; i < lefts.length; i++) {
                        if (lefts.filter(item => {
                            return item.parentId == lefts[i].id;
                        }) == 0 && this.data_raw.datas.filter(item => {
                            return item.parentId == lefts[i].id;
                        }) != 0) {
                            lefts.splice(i, 1);
                            i--;
